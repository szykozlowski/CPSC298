...
void Game::initializeGrid()
{
	...
	for (int iRow = 0; iRow < this->m_nGridRows;iRow++)
	{
		...
		{
			this->a_cGrid[iRow][iColumn] = '~';	/*?? assign '~' to element of 2-D array a_cGrid indexed by iRow and iColumn ?? */
		}
	}
	return;
}
...
bool Game::isCloseToTarget(int iRow, int iColumn)
{
	...
	// the target boat.
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
	if ((std::abs(iRow - this->m_iRowBoat) <=1 && std::abs(iRow - this->m_iRowBoat) >=0) && (std::abs(iColumn - this->m_iColumnBoat) <=1 && std::abs(iColumn - this->m_iColumnBoat) >= 0)
	{
		bClose = true;
	}
