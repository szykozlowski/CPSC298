...
std::string a_strNotes[] = {
    ...
    for(int nu = 0; nu < k_nOctaves; nu++){
      for (int k = 0; k < k_nHalfTones; k++){
        f = calculateFrequency(TWELFTH,REFERENCE,nu,k);
        dWavelengthCentimeters = calculateWavelength(SOUNDSPEED,calculateFrequency(TWELFTH,REFERENCE,nu,k));
          strNote = a_strNotes[nu * 9 + k];
          std::cout << "Note: " << strNote << "; nu: " << nu
          << "; k: " << k << "; frequency: " << f << " Hz; wavelength: "
